...
public class MonitorRelationController {
    ...
    public String add(HttpServletRequest request,CompanyMonitor companyMonitor) throws NoSuchAlgorithmException, UnsupportedEncodingException{
    	...
    	//找出上级monitorId,根据地址码找MonitorId
    	List<BigDecimal> superMonitorIds = new ArrayList<BigDecimal>();
    	superMonitorIds.add(new BigDecimal(maxId));
    	if(3==companyMonitor.getLevel()){
    		List<BigDecimal> mid = companyMonitorService.findIdByAreaCode(companyMonitor.getAreaCode().substring(0, 5)+"00");
    		if(null != mid) superMonitorIds.addAll(mid);
    		mid = companyMonitorService.findIdByAreaCode(companyMonitor.getAreaCode().substring(0, 3)+"0000");
    		if(null != mid) superMonitorIds.addAll(mid);
    	}else if(2==companyMonitor.getLevel()){
    		List<BigDecimal> mid = companyMonitorService.findIdByAreaCode(companyMonitor.getAreaCode().substring(0, 3)+"0000");
    		if(null != mid) superMonitorIds.addAll(mid);
    	}
    	...
    	try {
    		if(null != companyId && 0 != companyId.length){
	    		for(int i=0;i<companyId.length;i++){
	    			if(null!=superMonitorIds && 0!=superMonitorIds.size()){
	    				for(int j =0;j<superMonitorIds.size();j++){
	    					//监管部门ID，这个地方需要找出上级monitorID
	    	    			monitorRelation.setMonitorId(superMonitorIds.get(j));
	    	    			//被监管企业ID
